...
| A solutions architect is required to move 750 TB of data from a branch office's network-attached file system to Amazon S3 Glacier.
| Order 10 Snowball appliances and select S3 bucket to the destination

| HPC workload will use many Amazon EC2 instances and will generate a large quantity of small output files that must be stored in persistent storage for future use
|Fsx For lustre with S3

| The website’s users are located around the world and the solutions architect needs to optimize the performance for both the real-time and on-demand streaming.
|Cloudfront

| Amazon S3 bucket configured as an origin for an Amazon CloudFront distribution. Access to the static content must be restricted to the same set of IP addresses.
| Create origin access OAI and associate it with the distribution, change the permission in the bucket / Create a WAF web acl that includes the same IP restrictions associated with Cloudfront

| Amazon S3 bucket. The data is analyzed for the first week and then must remain immediately available for occasional analysis
| Configure lifecycle policy to transition the object to S2 Standard-Infrequent Access (S3 Standard-IA) after 30 days

| All levels of stored data must be audited under a new regulation which the company adheres to.
| Use storage Gateway to move existing data to S3. use CloutTrail to log management events

|Amazon Elastic File System (Amazon EFS). Which of the following performance modes should be used for Amazon EFS to address this requirement
|Max I/O

|The engineering team wants to decouple the user authentication process
|Cognito Authentication via Cognito User Pools ALB

| Distributed File System Replication (DFSR) service to keep files synchronized on multiple Windows servers, and would like to migrate to AWS cloud.

What do you recommend as a replacement for the DFSR?

|FSx for Windows File Server

|in a hub-and-spoke style using the AWS Transit Gateway
|Build a Shared services Virtual Private Cloud

|administrator has created a private hosted zone and associated it with a Virtual Private Cloud (VPC). However, the Domain Name System (DNS) queries for the private hosted zone remain unresolved
|Enable DNS hostnames and DNS resolution for private hosted zones

| team at a logistics company has noticed that the Auto Scaling group (ASG) is not terminating an unhealthy Amazon EC2 instance
| The health grace period, for instance

THe instance has failed the ELB health check status

The instance maybe in impaired status

|, the root volume of an Amazon EC2 instance for an EBS-backed AMI is deleted when the instance terminates.
|Set DeleteOnTermination attribute to false

|The IoT data is funneled into Amazon Kinesis Data Streams which further acts as the source of a delivery stream for Amazon Kinesis Firehose
|Kinesis Agent cannot write to Amazon Kinesis Firehose for which the delivery stream source is already set Kinesis DatStream
